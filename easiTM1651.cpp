...
void TM1651::begin(uint8_t numDigits, uint8_t brightness) {
  ...
  if(_numDigits > 3) {                                // If we have 4 digits, we cannot have a Gotek LEDC68 module.
    _LEDC68 = false
  }
...
void TM1651::displayChar(uint8_t digit, uint8_t number, bool raw) {
  if(digit > (_numDigits - 1)) {                        // Boundry check the digit number.
    digit = _numDigits - 1;
  }
...
void TM1651::displayInt8(uint8_t digit, uint8_t number, bool useDec) {
  if(_numDigits > 1) {                                  // We need at least 2 digits to display an 8-bit number.
    if(digit > (_numDigits - 1)) {
      digit = _numDigits - 1;
    }
    if(useDec) {
      if(number > 99) {
    ...
    this->stop();                                       // Send the stop signal to the TM1651.
    this->start();
    this->writeByte(STARTADDR + (digit - 1));           // Start at the first digit address.
...
void TM1651::displayInt12(uint8_t digit, uint16_t number, bool useDec) {
  if(_numDigits > 2) {                                  // We need at least 3 digits to display an 12-bit number.
...
void TM1651::displayInt16(uint8_t digit, uint16_t number, bool useDec) {
  if(_numDigits > 3) {                                  // We need at least 4 digits to display an 16-bit number.
...
void TM1651::displayDP(bool status) {
  if(_LEDC68) {                                           // If we have a Gotek LEDC68 module with DP control.
    if(_numDigits < 4) {                                  // The DP is controlled via digit 4, segment d, on the Gotek LEDC68 module.
      uint8_t segData;
      ...
      this->stop();                                       // Send the stop signal to the TM1651.
      this->start();
      this->writeByte(STARTADDR + 0x03);                  // Start address +0x03 controls the decimal points.
